...
import { O as OAuthProviders } from './internal-BfhZpZd3.js';
var n$2,
  l$3,
  u$2,
  i$1,
...
(function (exports) {
  ...
  function parseHexToInt(hex) {
    var isOnlyHexChars = !hex.match(/[^a-f0-9]/i);
    return isOnlyHexChars ? parseInt(hex, 16) : NaN;
  }
  function validateAndParseHex(hex, errorName, enforcedLength) {
    var parsedHex = parseHexToInt(hex);
  ...
  function parseHexadecimalCode(code) {
    var parsedCode = validateAndParseHex(code, errors_1.ErrorType.MalformedHexadecimal, 2);
    return String.fromCharCode(parsedCode);
  }
  function parseUnicodeCode(code, surrogateCode) {
    var parsedCode = validateAndParseHex(code, errors_1.ErrorType.MalformedUnicode, 4);
    if (surrogateCode !== undefined) {
      var parsedSurrogateCode = validateAndParseHex(surrogateCode, errors_1.ErrorType.MalformedUnicode, 4);
  ...
  function parseUnicodeCodePointCode(codePoint) {
    ...
    var withoutBraces = codePoint.slice(1, -1);
    var parsedCode = validateAndParseHex(withoutBraces, errors_1.ErrorType.MalformedUnicode);
    try {
  ...
  function parseOctalCode(code) {
    var error = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : false;
    if (error) {
      throw new SyntaxError(errors_1.errorMessages.get(errors_1.ErrorType.OctalDeprecation));
    }
    var parsedCode = parseInt(code, 8);
    return String.fromCharCode(parsedCode);
  }
  ...
  function unraw(raw) {
    var allowOctals = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : false;
...
(function (exports) {
  ...
  var moo_1 = __importDefault(mooExports);
  exports.states = {
    body: {
      doubleapos: {
        match: "''",
...
var DateFormatError = /*#__PURE__*/function (_Error2) {
  function DateFormatError(msg, token, type) {
    var _this;
    _classCallCheck(this, DateFormatError);
    _this = _callSuper(this, DateFormatError, [msg]);
    _this.token = token;
...
function weekdayStyle(token, onError) {
  var _char2 = token["char"],
    desc = token.desc,
    width = token.width;
  if ((_char2 === 'c' || _char2 === 'e') && width < 3) {
...
var isLetter = function isLetter(_char4) {
  return _char4 >= 'A' && _char4 <= 'Z' || _char4 >= 'a' && _char4 <= 'z';
};
function readFieldToken(src, pos) {
  var _char5 = src[pos];
  var width = 1;
  while (src[++pos] === _char5) ++width;
  var field = fields[_char5];
  if (!field) {
    var msg = "The letter ".concat(_char5, " is not a valid field identifier");
    return {
      "char": _char5,
      error: new Error(msg),
      width: width
...
function readToken(src, pos) {
  var _char6 = src[pos];
  if (!_char6) return null;
  if (isLetter(_char6)) return readFieldToken(src, pos);
  if (_char6 === "'") return readQuotedToken(src, pos);
  var str = _char6;
  var width = 1;
  while (true) {
    var next = src[++pos];
...
function processTokens(tokens, mapText) {
  ...
  return tokens.map(function (token) {
    if (token.type === "content") {
      ...
      var _token$param;
      var _param = token === null || token === void 0 || (_token$param = token.param) === null || _token$param === void 0 ? void 0 : _token$param[0];
      if (token.key === "date" && _param) {
        var opts = compileDateExpression(_param.value.trim(), function (e) {
          throw new Error("Unable to compile date expression: ".concat(e.message));
        });
    ...
    var offset = token.pluralOffset;
    var formatProps = {};
    token.cases.forEach(function (_ref2) {
      var key = _ref2.key,
        tokens2 = _ref2.tokens;
      var prop = key[0] === "=" ? key.slice(1) : key;
...
function compileDateExpression(format, onError) {
  if (/^::/.test(format)) {
    var tokens = parseDateTokens(format.substring(2));
    return getDateFormatOptions(tokens, void 0, onError);
  }
  return format;
}
function compileMessageOrThrow(message) {
...
var isFunction$1 = function isFunction$1(f) {
  return typeof f === "function";
};
var cache$1 = /* @__PURE__ */new Map();
var defaultLocale = "en";
function normalizeLocales(locales) {
  var out = Array.isArray(locales) ? locales : [locales];
  return [].concat(_toConsumableArray(out), [defaultLocale]);
}
function _date(locales, value, format) {
  var _locales = normalizeLocales(locales);
  if (!format) {
    format = "default";
  }
  var o;
  ...
  var formatter = getMemoized(function () {
    return cacheKey("date", _locales, format);
  }, function () {
    return new Intl.DateTimeFormat(_locales, o);
  });
  return formatter.format(isString(value) ? new Date(value) : value);
}
function _time(locales, value, format) {
  var o;
  if (!format) {
    format = "default";
  }
  ...
  return _date(locales, value, o);
}
function _number(locales, value, format) {
  var _locales = normalizeLocales(locales);
  var formatter = getMemoized(function () {
    return cacheKey("number", _locales, format);
  }, function () {
    return new Intl.NumberFormat(_locales, format);
  });
  return formatter.format(value);
}
function _plural(locales, ordinal, value, _ref3) {
  var _ref4, _rules$value;
  var _ref3$offset = _ref3.offset,
    offset = _ref3$offset === void 0 ? 0 : _ref3$offset,
    rules = _objectWithoutProperties(_ref3, _excluded);
  var _locales = normalizeLocales(locales);
  var plurals = ordinal ? getMemoized(function () {
    return cacheKey("plural-ordinal", _locales);
  }, function () {
    return new Intl.PluralRules(_locales, {
      type: "ordinal"
    });
  }) : getMemoized(function () {
    return cacheKey("plural-cardinal", _locales);
  }, function () {
    return new Intl.PluralRules(_locales, {
      type: "cardinal"
    });
  });
  return (_ref4 = (_rules$value = rules[value]) !== null && _rules$value !== void 0 ? _rules$value : rules[plurals.select(value - offset)]) !== null && _ref4 !== void 0 ? _ref4 : rules.other;
}
function getMemoized(getKey, construct) {
  var key = getKey();
  var formatter = cache$1.get(key);
  if (!formatter) {
    formatter = construct();
    cache$1.set(key, formatter);
  }
  return formatter;
}
function cacheKey(type, locales, options) {
  var localeKey = locales.join("-");
  return "".concat(type, "-").concat(localeKey, "-").concat(JSON.stringify(options));
}
var UNICODE_REGEX = /\\u[a-fA-F0-9]{4}|\\x[a-fA-F0-9]{2}/;
var OCTOTHORPE_PH = "%__lingui_octothorpe__%";
var getDefaultFormats = function getDefaultFormats(locale, passedLocales) {
  var formats = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : {};
  var locales = passedLocales || locale;
  var style = function style(format) {
    if (_typeof(format) === "object") return format;
    return formats[format];
  };
  var replaceOctothorpe = function replaceOctothorpe(value, message) {
    var numberFormat = Object.keys(formats).length ? style("number") : void 0;
    var valueStr = _number(locales, value, numberFormat);
    return message.replace(new RegExp(OCTOTHORPE_PH, "g"), valueStr);
  };
  return {
    plural: function plural(value, cases) {
      var _cases$offset = cases.offset,
        offset = _cases$offset === void 0 ? 0 : _cases$offset;
      var message = _plural(locales, false, value, cases);
      return replaceOctothorpe(value - offset, message);
    },
    selectordinal: function selectordinal(value, cases) {
      var _cases$offset2 = cases.offset,
        offset = _cases$offset2 === void 0 ? 0 : _cases$offset2;
      var message = _plural(locales, true, value, cases);
      return replaceOctothorpe(value - offset, message);
    },
    select: selectFormatter,
    number: function number(value, format) {
      return _number(locales, value, style(format) || {
        style: format
      });
    },
    date: function date(value, format) {
      return _date(locales, value, style(format) || format);
    },
    time: function time(value, format) {
      return _time(locales, value, style(format) || format);
    }
  };
};
var selectFormatter = function selectFormatter(value, rules) {
  var _rules$value2;
  return (_rules$value2 = rules[value]) !== null && _rules$value2 !== void 0 ? _rules$value2 : rules.other;
};
function interpolate(translation, locale, locales) {
  return function () {
    var values = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : {};
    var formats = arguments.length > 1 ? arguments[1] : undefined;
    var formatters = getDefaultFormats(locale, locales, formats);
    var _formatMessage = function formatMessage(tokens) {
      var replaceOctothorpe = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : false;
      if (!Array.isArray(tokens)) return tokens;
      return tokens.reduce(function (message, token) {
        ...
        if (type === "plural" || type === "selectordinal" || type === "select") {
          Object.entries(format).forEach(function (_ref5) {
            var _ref6 = _slicedToArray(_ref5, 2),
              key = _ref6[0],
              value2 = _ref6[1];
            interpolatedFormat[key] = _formatMessage(value2, type === "plural" || type === "selectordinal");
          });
        } else {
    ...
    var result = _formatMessage(translation);
    if (isString(result) && UNICODE_REGEX.test(result)) {
      return dist.unraw(result);
    }
  ...
  return _createClass(EventEmitter, [{
    ...
    value: function on(event, listener) {
      var _a$event,
        _this2 = this;
      var _a;
      (_a$event = (_a = this._events)[event]) !== null && _a$event !== void 0 ? _a$event : _a[event] = [];
      this._events[event].push(listener);
      return function () {
        return _this2.removeListener(event, listener);
      };
    }
  }, {
    key: "removeListener",
    value: function removeListener(event, listener) {
      var maybeListeners = this._getListeners(event);
      if (!maybeListeners) return;
      var index = maybeListeners.indexOf(listener);
    ...
    value: function emit(event) {
      ...
      var maybeListeners = this._getListeners(event);
      if (!maybeListeners) return;
      maybeListeners.map(function (listener) {
    ...
    value: function _getListeners(event) {
      var maybeListeners = this._events[event];
...
var I18n = /*#__PURE__*/function (_EventEmitter) {
  function I18n(params) {
    ...
    _this4 = _callSuper(this, I18n);
    __publicField(_this4, "_locale", "");
    __publicField(_this4, "_locales");
    __publicField(_this4, "_localeData", {});
    __publicField(_this4, "_messages", {});
    __publicField(_this4, "_missing");
    __publicField(_this4, "_messageCompiler");
    __publicField(_this4, "t", _this4._.bind(_this4));
    if (params.missing != null) _this4._missing = params.missing;
    if (params.messages != null) _this4.load(params.messages);
    if (params.localeData != null) _this4.loadLocaleData(params.localeData);
    if (typeof params.locale === "string" || params.locales) {
      var _params$locale;
      _this4.activate((_params$locale = params.locale) !== null && _params$locale !== void 0 ? _params$locale : defaultLocale, params.locales);
    }
    return _this4;
  }
  ...
  return _createClass(I18n, [{
    ...
    value: function _loadLocaleData(locale, localeData) {
      var maybeLocaleData = this._localeData[locale];
      if (!maybeLocaleData) {
    ...
    key: "setMessagesCompiler",
    value: function setMessagesCompiler(compiler) {
    ...
    value: function loadLocaleData(localeOrAllData, localeData) {
      ...
      if (typeof localeOrAllData === "string") {
        ...
        Object.keys(localeOrAllData).forEach(function (locale) {
          return _this5._loadLocaleData(locale, localeOrAllData[locale]);
        });
      }
      this.emit("change");
    }
    ...
    value: function _load(locale, messages) {
      var maybeMessages = this._messages[locale];
      if (!maybeMessages) {
    ...
    value: function load(localeOrMessages, messages) {
      var _this6 = this;
      if (typeof localeOrMessages == "string" && _typeof(messages) === "object") {
        this._load(localeOrMessages, messages);
      } else {
        Object.entries(localeOrMessages).forEach(function (_ref7) {
          var _ref8 = _slicedToArray(_ref7, 2),
            locale = _ref8[0],
            messages2 = _ref8[1];
          return _this6._load(locale, messages2);
        });
      }
      this.emit("change");
    }
    ...
    key: "loadAndActivate",
    value: function loadAndActivate(_ref9) {
    ...
    key: "_",
    value: function _(id, values, options) {
      if (!this.locale) {
        throw new Error("Lingui: Attempted to call a translation function without setting a locale.\nMake sure to call `i18n.activate(locale)` before using Lingui functions.\nThis issue may also occur due to a race condition in your initialization logic.");
      }
      ...
      var messageForId = this.messages[id];
      var messageMissing = messageForId === void 0;
      ...
      var translation = messageForId || message || id;
      ...
      return interpolate(translation, this._locale, this._locales)(values, options === null || options === void 0 ? void 0 : options.formats);
    }
  }, {
    key: "date",
    value: function date(value, format) {
      return _date(this._locales || this._locale, value, format);
    }
  }, {
    key: "number",
    value: function number(value, format) {
      return _number(this._locales || this._locale, value, format);
    }
  }]);
}(EventEmitter);
function setupI18n() {
...
function formatElements(value) {
  ...
  var _iterator5 = _createForOfIteratorHelper(getElements(parts)),
    _step5;
  try {
    for (_iterator5.s(); !(_step5 = _iterator5.n()).done;) {
      var _step5$value = _slicedToArray(_step5.value, 3),
        ...
        after = _step5$value[2];
      var element = typeof index !== "undefined" ? elements[index] : void 0;
      if (!element || voidElementTags[element.type] && children) {
        if (!element) {
          console.error("Can't use element at index '".concat(index, "' as it is not declared in the original translation"));
        } else {
          console.error("".concat(element.type, " is a void element tag therefore it must have no children"));
        }
        element = wn.createElement(wn.Fragment);
      }
      if (Array.isArray(element)) {
        element = wn.createElement(wn.Fragment, {}, element);
      }
      tree.push(wn.cloneElement(element, {
        key: uniqueId()
...
function getElements(parts) {
  ...
  var triple = [paired || unpaired, children || "", after];
...
function TransNoContext(props) {
  var render = props.render,
    ...
    _props$lingui = props.lingui,
    i18n = _props$lingui.i18n,
    defaultComponent = _props$lingui.defaultComponent;
  ...
  if (values) {
    Object.keys(values).forEach(function (key) {
      ...
      components[index] = /* @__PURE__ */wn.createElement(wn.Fragment, null, values[key]);
  ...
  var FallbackComponent = defaultComponent || RenderFragment;
  var i18nProps = {
    id: id,
    message: message,
  ...
  if (render && component) {
    ...
    return wn.createElement(FallbackComponent, i18nProps, translation);
  }
  if (typeof render === "function") {
    return render(i18nProps);
  }
  var Component = component || FallbackComponent;
  return wn.createElement(Component, i18nProps, translation);
}
var RenderFragment = function RenderFragment(_ref0) {
  var children = _ref0.children;
  return /* @__PURE__ */wn.createElement(wn.Fragment, null, children);
};
var LinguiContext = wn.createContext(null);
var useLinguiInternal = function useLinguiInternal(devErrorMessage) {
  var context = wn.useContext(LinguiContext);
  return context;
};
function useLingui() {
  return useLinguiInternal();
}
var I18nProvider = function I18nProvider(_ref1) {
  var i18n = _ref1.i18n,
    defaultComponent = _ref1.defaultComponent,
    children = _ref1.children;
  var latestKnownLocale = wn.useRef(i18n.locale);
  var makeContext = wn.useCallback(function () {
    return {
      i18n: i18n,
  ...
  var _wn$useState = wn.useState(makeContext()),
    _wn$useState2 = _slicedToArray(_wn$useState, 2),
    context = _wn$useState2[0],
    setContext = _wn$useState2[1];
  wn.useEffect(function () {
    var updateContext = function updateContext() {
      latestKnownLocale.current = i18n.locale;
      setContext(makeContext());
    };
    var unsubscribe = i18n.on("change", updateContext);
  ...
  return /* @__PURE__ */wn.createElement(LinguiContext.Provider, {
    value: context
  }, children);
};
function Trans(props) {
  var lingui = useLinguiInternal();
  return wn.createElement(TransNoContext, _objectSpread(_objectSpread({}, props), {}, {
    lingui: lingui
  }));
}
var I18nProviderWrapper = function I18nProviderWrapper(_ref10) {
  var children = _ref10.children,
    messages = _ref10.messages,
    overrides = _ref10.overrides;
  ...
  return /*#__PURE__*/wn.createElement(I18nProvider, {
    i18n: i18n
  }, children);
};
...
function CreateWebComponent(ReactComponent, webComponentName) {
  ...
  var ReactWebComponent = /*#__PURE__*/function (_HTMLElement) {
    function ReactWebComponent(props) {
      ...
      _this7 = _callSuper(this, ReactWebComponent);
      _this7.props = props;
      _this7.rootRef = document.createElement('div');
      _this7.append(_this7.rootRef);
      return _this7;
    }
...
function CreateShadowWebComponent(ReactComponent, webComponentName) {
  ...
  var ReactWebComponent = /*#__PURE__*/function (_HTMLElement2) {
    function ReactWebComponent(props) {
      var _this8;
      _classCallCheck(this, ReactWebComponent);
      _this8 = _callSuper(this, ReactWebComponent);
      _this8.props = props;
      _this8.rootRef = _this8.attachShadow({
        mode: 'open'
      });
      return _this8;
    }
...
var _ref11 = isWindowDefined && window.addEventListener ? [window.addEventListener.bind(window), window.removeEventListener.bind(window)] : [noop, noop],
  _ref12 = _slicedToArray(_ref11, 2),
  onWindowEvent = _ref12[0],
  offWindowEvent = _ref12[1];
...
var initCache = function initCache(provider, options) {
  if (!SWRGlobalState.has(provider)) {
    ...
    var setter = function setter(key, value, prev) {
      ...
      var subs = subscriptions[key];
      if (subs) {
        var _iterator6 = _createForOfIteratorHelper(subs),
          _step6;
        try {
          for (_iterator6.s(); !(_step6 = _iterator6.n()).done;) {
            var _fn = _step6.value;
            _fn(value, prev);
          }
        } catch (err) {
          _iterator6.e(err);
...
var useSWRHandler = function useSWRHandler(_key, fetcher, config) {
  ...
  var revalidate = T$2(/*#__PURE__*/function () {
    var _ref13 = _asyncToGenerator(/*#__PURE__*/_regeneratorRuntime().mark(function _callee(revalidateOpts) {
...
export { fn as A, y$1 as B, Cn as C, ln as D, l$3 as E, F$2 as F, A$2 as G, He as H, I18nProviderWrapper as I, sn as J, A$3 as K, j$1 as L, unstable_serialize as M, N$1 as N, createCacheHelper as O, isUndefined as P, shimExports as Q, React as R, isFunction as S, T$2 as T, Ue as U, Ze as Z, _$1 as _, withMiddleware as a, useSWR as b, useSWRConfig as c, useStateWithDeps as d, mergeObjects as e, UNDEFINED as f, getTimestamp as g, useIsomorphicLayoutEffect as h, isTruthy as i, h$3 as j, p$2 as k, g$4 as l, merge as m, Trans as n, w$2 as o, passwordManagerDisableAutofillProps as p, q$4 as q, extractErrorMessage as r, serialize as s, F$3 as t, useLingui as u, s as v, wn as w, CreateSSRSafeWebComponent as x, y$2 as y, Fe as z };
